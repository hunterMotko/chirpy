...
import { makeJWT, validateJWT, hashPassword, comparePassword, extractBearerToken } from "./auth.js";
...
describe("Password Hashing", () => {
	...
	it("should return false for an incorrect password", async () => {
		const result = await comparePassword("wrongPassword", hash1);
		expect(result).toBe(false);
	});

	it("should return false when password doesn't match a different hash", async () => {
		const result = await comparePassword(password1, hash2);
		expect(result).toBe(false);
	});

	it("should return false for an empty password", async () => {
		const result = await comparePassword("", hash1);
		expect(result).toBe(false);
	});

	it("should return false for an invalid hash", async () => {
		const result = await comparePassword(password1, "invalidhash");
		expect(result).toBe(false);
	});
});
